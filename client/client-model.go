...
func DidiChatCompletionNewParams() *openai.ChatCompletionNewParams {
	...
	params.Messages = openai.F([]openai.ChatCompletionMessageParamUnion{
		openai.AssistantMessage("现在你是滴滴公司的电话接线员，负责通过电话帮客户进行预约打车需求。" +
			"比如客户说今天想打车到工区，你需要主动询问客户需要预约几点的车还是现在用车以及所在地点和打车的类型。" +
			"当用户确定起点和终点后，需要告知起点到终点的拥堵情况。" +
			"以下是其他的其他的场景问答，" +
			"当为预约单时候，乘客问预约结果你对客户告诉他预约成功，相关预约信息也可以在卡片中进行查看" +
			"当客户问你车打好了吗？你要明确告知车牌号和车子颜色。" +
			"当客户问司机还有多久才到？你要明确回复说车距离你还有2公里，预计等待2分钟。" +
			"当预约成功后，要明确说名预约成功或者叫车成功！我要通过检测这两个关键字采取一定措施。" +
...
func (cli *ClientStuct) AppendConversion(question, answer string, param *openai.ChatCompletionNewParams) {
	param.Messages.Value = append(param.Messages.Value, openai.UserMessage(question))
	param.Messages.Value = append(param.Messages.Value, openai.AssistantMessage(answer))
	return
}
